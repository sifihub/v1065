...
PROMO_LINKS = (
    "https://sifihub.github.io/AVENZIE",
    "https://avenzie1.wed2c.com/searchPage?title=Shop%20Now",
)

PROMO_CTA_CLEANUP_RE = re.compile(
    r"\b(?:come\s+)?check\s+(?:my|the|our)?\s*pinned\s+comment\b"
    r"|\bget\s+coupons?\s+huge\s+among\s+all\b"
    r"|\bpinned\s+comment\b"
    r"|\bcoupons?\s+huge\b",
    re.IGNORECASE,
)

PUBLIC_URL_RE = re.compile(r"https?://\S+|(?:www\.)?\b(?:sifihub\.github\.io/AVENZIE|avenzie1\.wed2c\.com/\S*)", re.IGNORECASE)
...
class ZaraAI:
    ...
    def _seed_initial_beliefs(self) -> None:
        beliefs = [
            ("Zara's public lanes are fashion, women's wear, beauty, and Avenzie store promotion.", "core", 0.94),
            ...
            ("Geopolitics, war, and crypto language is always off-mission.", "core", 0.93),
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="zara_cpr_") as temp_dir:
                ...
                result = subprocess.run(
                    ...
                    timeout=max(120, _env_int("ZARA_CPR_CLONE_TIMEOUT_SECONDS", 300)),
                )
    ...
    def _ask_browser_llm(self, prompt: str, prefer: str = "deepseek") -> Optional[str]:
        if self.dry_run or self.browser.driver is None or not prompt.strip():
            return None
        fallback_order = ["gemini", "chatgpt", "deepseek"]
        preferred = (prefer or "").strip().lower()
        ordered = [preferred] if preferred in fallback_order else []
        ordered.extend(target for target in fallback_order if target not in ordered)
    ...
    def _with_promo_link(self, text: str, *, limit: int = 280) -> str:
        cleaned = PROMO_CTA_CLEANUP_RE.sub("", text or "")
        cleaned = PUBLIC_URL_RE.sub("", cleaned)
        cleaned = re.sub(r"\s+([?.!,])", r"\1", cleaned)
        cleaned = " ".join(cleaned.split()).strip(" -:|")
        cleaned = re.sub(r"^(?:and|also|plus)[\s.,:;-]+", "", cleaned, flags=re.IGNORECASE).strip()
        if not cleaned:
            return ""
        link = random.choice(PROMO_LINKS)
        max_text_len = max(40, limit - len(link) - 1)
        return f"{_truncate(cleaned, max_text_len)} {link}".strip()
    ...
    def _question_suffix(self, topic: str) -> str:
        ...
        if any(token in topic_key for token in ("fashion", "outfit", "style", "wear")):
            return "How would you style this differently?"
        if any(token in topic_key for token in ("beauty", "makeup", "hair")):
            return "What's the real secret behind this look?"
        if any(token in topic_key for token in ("shopping", "store", "deal", "coupon")):
            return "Who else is hunting for these Avenzie coupons?"
        if any(token in topic_key for token in ("trend", "viral", "popular")):
            return "Is this going to be the biggest trend of the season?"
        return "What do you think is the best way to pull this off?"
    ...
    def _candidate_quality_score(self, candidate: dict) -> float:
        ...
        if captured_at:
            ...
            try:
                ...
                if age_hours > 12.0 and replies < 10:
                    score -= 50.0 # Stale
                elif age_hours < 6.0:
                    score += 15.0 # Fresh
            except Exception:
    ...
    def _maybe_address_author(self, reply: str, candidate: dict) -> str:
        ...
        return self._public_safe_text(f"{handle} {compact}", limit=180) or reply
    ...
    def _ensure_engagement_question(self, reply: str, candidate: dict) -> str:
        ...
        suffix = self._question_suffix(str(candidate.get("topic", "")))
        joined = f"{base}. {suffix}" if base else suffix
        return self._public_safe_text(joined, limit=180) or reply
    ...
    def _generate_engagement_comment(self, candidate: dict) -> str:
        ...
        if rand_val < 0.7:
            style = random.choice(["challenge", "question", "witty_observation", "counterpoint", "useful_context", "prediction"])
        elif rand_val < 0.9:
            style = "soft_promo"
        else:
            style = "direct_promo"
        candidate["comment_style"] = style
        ...
        reply = self._public_safe_text(reply, limit=180)
        if not reply:
            reply = self._fallback_engagement_comment(candidate)
        ...
        reply = self._with_topic_hashtags(
            reply,
            str(candidate.get("topic", "")).strip() or "general",
            limit=180,
            max_tags=2,
        )
        return self._with_promo_link(reply, limit=280)
    ...
    def generate_and_maybe_post(self) -> dict:
        log.info("Original posts disabled for Avenzie marketing mode.")
        return {'status': 'skip', 'reason': 'comments only mode'}
    ...
    def engage_with_trending_posts(self, max_comments_override: int | None = None) -> dict:
        ...
        max_comments_cap = max(1, _env_int("ZARA_MAX_TREND_COMMENTS_PER_CYCLE_CAP", 30))
        configured_max = max_comments_override if max_comments_override is not None else _env_int("ZARA_MAX_TREND_COMMENTS_PER_CYCLE", 15)
        ...
        for candidate in candidates:
            ...
            if posted or self.dry_run or not publish_enabled:
    ...
    def analyze_notifications(self) -> dict:
        ...
        recent_engagements = self.memory.get_recent_engaged_sources(limit=100)
        
        for item in notifications:
            kind = str(item.get("kind", "notification"))
            summary["kinds"][kind] = summary["kinds"].get(kind, 0) + 1
            
            user = str(item.get("user", "")).strip().lower()
            url = str(item.get("url", ""))
            
            if kind in ("like", "reply", "follow", "repost") and user:
                for src in recent_engagements:
                    meta = src.get("metadata", {})
                    src_author = str(meta.get("author_handle", "")).strip().lower()
                    if src_author == user or (url and url in str(src.get("source_url", ""))):
                        keys = [src["canonical_key"]]
                        placeholders = ",".join("?" for _ in keys)
                        cur = self.memory.conn.cursor()
                        if kind == "like":
                            cur.execute(f"UPDATE engaged_sources SET received_likes = received_likes + 1 WHERE canonical_key IN ({placeholders})", keys)
                        elif kind == "reply":
                            cur.execute(f"UPDATE engaged_sources SET received_replies = received_replies + 1, needs_followup = 1 WHERE canonical_key IN ({placeholders})", keys)
    ...
    def check_mentions(self) -> int:
        ...
        for mention in mentions[:5]:
            ...
            reply = self._public_safe_text(self.llm.ask(prompt, timeout=120, role="creator"), limit=180)
            if not reply:
                reply = self._public_safe_text(self._ask_browser_llm(prompt, prefer="deepseek") or "The quiet part is the timing. Everyone is arguing the headline while the map moves underneath.", limit=180)
            
            reply = self._with_topic_hashtags(reply, "discussion", limit=180, max_tags=2)
            reply = self._with_promo_link(reply, limit=280)
    ...
    def run_forever(self, hours_per_run: float = 5.25) -> dict:
        ...
        if getattr(self, "rebirth_in_progress", False):
            return {"mode": "watchdog_hijacked", "iteration": self.iteration}
