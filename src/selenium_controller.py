...
# Skipping broken Ctrl+Enter, clicking button directly:
    ...
    def reply_to_tweet(self, tweet_url: str, reply_text: str) -> bool:
        ...
        def _composer_text(element) -> str:
            try:
                return str(self.driver.execute_script("return arguments[0].innerText || arguments[0].textContent || '';", element) or "")
            except Exception:
                return ""

        def _reply_looks_submitted() -> bool:
            try:
                boxes = self.driver.find_elements(By.CSS_SELECTOR, "[data-testid='tweetTextarea_0'], div[role='textbox']")
                if boxes and any(safe_text[:40] in _composer_text(box) for box in boxes):
                    return False
            except StaleElementReferenceException:
                return True
            except Exception:
        ...
        def _find_enabled_reply_button(box):
            buttons = self.driver.find_elements(
                By.CSS_SELECTOR,
                "[data-testid='tweetButtonInline'], [data-testid='postButtonInline'], [data-testid='tweetButton'], [data-testid='postButton']",
            )
            usable = []
            for button in buttons:
                try:
                    ...
                    distance = 0
                    try:
                        distance = abs(float(button.rect.get("y", 0)) - float(box.rect.get("y", 0)))
                    except Exception:
                        pass
                    usable.append((distance, button))
                except StaleElementReferenceException:
                    continue
                except Exception:
                    continue
        ...
        def _fill_composer(box) -> bool:
            ...
            if safe_text[:40] not in _composer_text(box):
            ...
            try:
                ...
                pass
            return safe_text[:40] in _composer_text(box)

        def _submit_reply_from_current_page(label: str) -> bool:
            selectors = ["[data-testid='tweetTextarea_0']", "div[role='textbox']"]
            for attempt in range(1, 4):
                try:
                    box = self._find_first(selectors, timeout=20)
                    if box is None:
                        log.error("Could not find reply composer on %s", label)
                        return False
                    if not _fill_composer(box):
                        log.warning("Reply composer did not retain text on %s attempt %s", label, attempt)
                        _delay(1, 2)
                        continue

                    _delay(1, 2)
                    button = _find_enabled_reply_button(box)
                    if button is None:
                        log.error("Reply button never became enabled on %s", label)
                        return False
                    ...
                    _delay(4, 6)
                    if _reply_looks_submitted():
                        log.info("Reply submission accepted through %s", label)
                        return True

                    try:
                        fresh_box = self._find_first(selectors, timeout=3)
                        if fresh_box is not None and safe_text[:40] in _composer_text(fresh_box):
                            fresh_box.send_keys(Keys.CONTROL, Keys.RETURN)
                    except Exception:
                        pass
                    _delay(4, 6)
                    if _reply_looks_submitted():
                        log.info("Reply submission accepted through %s keyboard fallback", label)
                        return True

                    log.warning("Reply text still present after %s submit attempt %s; retrying", label, attempt)
                except StaleElementReferenceException as exc:
                    log.warning("Reply composer changed during %s attempt %s; retrying: %s", label, attempt, exc)
                    _delay(1, 2)
                    continue
                except (ElementClickInterceptedException, ElementNotInteractableException, TimeoutException) as exc:
                    log.warning("Reply submit attempt %s on %s failed; retrying: %s", attempt, label, exc)
                    _delay(1, 2)
                    continue
            return False
        ...
        try:
            ...
            if _submit_reply_from_current_page("tweet page"):
                return True
            ...
            if status_id:
                intent_url = (
                    ...
                    f"in_reply_to={urllib.parse.quote(status_id)}&"
                    f"text={urllib.parse.quote(safe_text)}"
                )
                self.driver.get(intent_url)
                _delay(4, 6)
                if _submit_reply_from_current_page("reply intent"):
                    return True

            log.error("Reply submit failed after retries")
            return False
        except Exception as exc:
