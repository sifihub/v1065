...
class MemorySystem:
    ...
    def _init_database(self) -> None:
        ...
        try:
            c = self.conn.cursor()
            c.execute("ALTER TABLE engaged_sources ADD COLUMN received_likes INTEGER DEFAULT 0")
            c.execute("ALTER TABLE engaged_sources ADD COLUMN received_replies INTEGER DEFAULT 0")
            c.execute("ALTER TABLE engaged_sources ADD COLUMN needs_followup BOOLEAN DEFAULT 0")
            c.execute("ALTER TABLE engaged_sources ADD COLUMN followup_done BOOLEAN DEFAULT 0")
            c.execute("ALTER TABLE engaged_sources ADD COLUMN last_checked_at TIMESTAMP")
            c.execute("ALTER TABLE engaged_sources ADD COLUMN comment_style TEXT")
            self.conn.commit()
        except Exception:
            pass
